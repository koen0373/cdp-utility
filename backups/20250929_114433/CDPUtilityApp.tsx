...
const AssetRow: React.FC<{
  ...
  onUpdate: (qty: number) => void;
  onRemove: () => void;
  isLoan?: boolean; // Add prop to identify if this is a loan
}> = ({ row, value, onUpdate, onRemove, isLoan = false }) => {
  const [isEditing, setIsEditing] = useState(false);
  ...
  return (
    <div className="py-5">
      <div className="grid grid-cols-12 gap-2 items-center">
        ...
        >
          {isLoan ? `-${fmtUSD(value)}` : fmtUSD(value)}
        </div>
        <div className="col-span-1 flex items-center justify-end gap-2">
          {!isEditing ? (
            <>
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-8">
        ...
        <section className="card mb-6">
          ...
          {(() => {
            ...
            // Function to calculate compound interest for each holding
            ...
            // Function to get interest period in days from interestRate
            function getInterestPeriod(interestRate?: string): number {
              if (!interestRate) return 365;
              ...
              if (interestRate.includes('21') || interestRate.includes('24') || interestRate.includes('15')) return 91; // QUARTERLY
              if (interestRate.includes('22') || interestRate.includes('25') || interestRate.includes('16')) return 182; // SEMI-ANNUAL
              if (interestRate.includes('24') || interestRate.includes('27') || interestRate.includes('18')) return 365; // ANNUAL
              ...
              <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                  {upcomingPayouts.map((payout, i) => {
                    const assetPrice = payout.asset.symbol === 'COINDEPO' ? 0.11 : 
                        ...
                        <div className="grid grid-cols-11 gap-2 items-center">
                          ...
                          <div className="col-span-3 text-right">
                            <div className={`${payout.isLoan ? 'text-red-600' : 'text-green-600'} font-semibold`}>
                              {payout.isLoan ? '-' : '+'}{(calculation.accruedInterest / (payout.asset.symbol === 'COINDEPO' ? 0.11 : 
                                (rows.find(r => r.asset.symbol === payout.asset.symbol)?.priceUSD || 
        ...
        <div className="flex flex-row gap-6 mt-6">
          <section className="card flex-1">
            <div className="flex justify-between items-center mb-6">
              <h2 className="cd-balance-medium text-blue-600">Token Advantage Program</h2>
              <div className="flex items-center gap-2">
                  <input
                  type="checkbox"
                  id="extraPayout"
                ...
                />
                <label htmlFor="extraPayout" className="text-sm font-medium text-slate-700">
                  +2% Extra COINDEPO Token Payout
                </label>
              </div>
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold">
                    +{(currentTier.tokenPayout * 100).toFixed(0)}%
                    {extraPayoutEnabled && <span className="text-green-600"> (Enabled)</span>}
                  </span>
                </div>
              </div>
            </div>
          ...
          <section className="card flex-1">
            ...
            {otherValueUSD <= 0 ? (
              ...
              [
                { label: "Tier 1 (0-4.99%)", pct: 0.00, benefits: "2% token payout" },
                { label: "Tier 2 (5-9.99%)", pct: 0.05, benefits: "+1% deposit, -1% loan, 2% token" },
                { label: "Tier 3 (10-14.99%)", pct: 0.10, benefits: "+2% deposit, -2% loan, 2% token" },
                { label: "Tier 4 (15%+)", pct: 0.15, benefits: "+3% deposit, -3% loan, 2% token" },
              ].map(({ label, pct, benefits }) => {
                ...
                return (
                  <div key={label} className={`mb-3 p-3 rounded-lg ${achieved ? "bg-green-50 border border-green-200" : "bg-slate-50 border border-slate-200"}`}>
                    <div className={`font-semibold mb-1 ${achieved ? "text-green-600" : "text-slate-800"}`}>
                      {label}
                    </div>
                    <div className="text-sm text-slate-600 mb-2">
                      {benefits}
                    </div>
                    <div className={achieved ? "text-green-600" : "text-slate-700"}>
                      {achieved ? (
                        <>✔️ Achieved</>
                      ) : needTokens === null ? (
                        ...
                        <>
                          Need: {needTokens.toLocaleString()} COINDEPO{" "}
